...
export function registerLangSettings(pi: ExtensionAPI, deps: SettingsDeps): void {
	...
	const applyToggle = (ctx: ExtensionContext, cfg: Config, key: "check" | "auto" | "context", on: boolean) => {
		if (key === "check") {
			cfg.enabled = on;
	...
	const openSettingsMenu = async (ctx: ExtensionContext, cfg: Config) => {
		await ctx.ui.custom((tui, theme, _kb, done) => {
			...
			const languageSubmenu = (title: string, currentValue: string, submenuDone: (value?: string) => void) => {
				let mode: "list" | "input" = "list";

				const options: SelectItem[] = [
					...LANGUAGE_PRESETS.map((p) => ({ value: p.code, label: p.code, description: p.name })),
					{ value: CUSTOM_LANG, label: "custom…", description: "Type any language code (e.g. it, vi, th)" },
				];
				...
				return {
					render: (width: number) => [
						...
						...(mode === "list"
							? list.render(width)
							: [theme.fg("muted", " Language code (enter to confirm, esc to go back):"), ...input.render(width)]),
					],
					invalidate: () => {},
			...
			const modelSubmenu = (currentValue: string, submenuDone: (value?: string) => void) => {
				...
				list.onCancel = () => submenuDone(undefined);
				return {
					render: (width: number) => [theme.fg("accent", theme.bold(" Model")), "", ...list.render(width)],
					invalidate: () => {},
			...
			const items: SettingItem[] = [
				...
				},
				{
					...
					submenu: (current, submenuDone) => languageSubmenu("Native language", current, submenuDone),
				},
				{
					id: "learning",
					...
					submenu: (current, submenuDone) => languageSubmenu("Learning language", current, submenuDone),
				},
				{
					id: "model",
					label: "Model",
					currentValue: cfg.model ?? "default",
					description: 'Model used for checks and translations — "default" follows the session model',
	...
	pi.registerCommand("lang", {
		description: "Language learning: /lang opens the settings menu; /lang <key> <value> sets directly",
			...
			switch (sub) {
				...
				case "model":
					...
					if (value === "default") {
						...
						if (!hasAuth) {
							ctx.ui.notify(`No auth configured for ${value} — run /login ${found.provider} first`, "warning");
							return;
						}
						cfg.model = value;
