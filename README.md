...
/lang on | off             pause/resume the grammar check
...
To avoid wasted tokens and noise, the grammar check skips: slash/bang commands, messages under 4 words, messages that are mostly code or paths, messages not written in the learning language, and everything while `/lang off`. Checks run only in interactive TUI mode, and a failed check never disturbs your session.
