...
<p align="center">
  ...
  这是一个 <a href="https://pi.dev">pi</a> 扩展：检查你的 prompt、在你退回母语时教你怎么说，并把 agent 的回复变成沉浸式双语对照。
</p>
...
3. agent 回答完，按 `alt+t`（macOS 需在终端里[把 Option 设为 Meta](https://iterm2.com/documentation-preferences-profiles-keys.html)，也可直接 `/translate`）。回复会渲染成双语卡片。
...
跳过规则、check / tutor 分工、双语卡片细节、自定义 provider，以及上下文模式的成本权衡：

→ **[docs/advanced.zh-CN.md](docs/advanced.zh-CN.md)**
...
欢迎 PR。分支与提交约定见 [AGENTS.md](AGENTS.md)。
